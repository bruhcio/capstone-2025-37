...
public class NetworkingManager : MonoBehaviour
{
    ...
    public void SendRequest(PacketData packet){
        if(requestBlockingManager.IsRequestBlocked(packet.httpPath)){
    ...
    private UnityWebRequest Send(PacketData packet)
    {
        ...
        if (packet.requestType == RequestType.GET)
        {
            ...
            request = UnityWebRequest.Get(serverUrl + packet.httpPath + queries);
        }
        else
        {
            ...
            {
                string dataKey = packet.data.keys[i];
                postBody.AddField(dataKey, packet.data[dataKey].ToString());
            }

            request = UnityWebRequest.Post(serverUrl + packet.httpPath, postBody);
        }
